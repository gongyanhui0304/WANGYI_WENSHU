SELECT 
  A.部门 , A.年份月份 , A.各部门总数 , B.总数 , C.天数 , A.登录次数  
 ...
 FROM ( 
SELECT KOSTL AS "部门", LEFT(SAL_DATE,6) AS "年份月份"  
       , SUM(USERNUMBER_KY) AS "各部门总数"   
       , SUM(LOGNUMBER) AS "登录次数"        
FROM express_ods.dbo.ODS_SAP_ZBIT_LOG_BM  
WHERE LEFT(SAL_DATE,6) = ${年月} -- '202510' 
      AND KOSTL IS NOT NULL AND KOSTL <> '' AND KOSTL <> '信息中心' 
GROUP BY  KOSTL , LEFT(SAL_DATE,6) 
    ) AS A  
  LEFT JOIN ( 
...
GROUP BY C_YEAR_MONTH   ) AS C ON A.年份月份 = C.C_YEAR_MONTH
...
SELECT KOSTL AS "部门", LEFT(SAL_DATE,6) AS "年份月份"  
       , SUM(USERNUMBER_KY) AS "各部门总数"   
       , SUM(LOGNUMBER) AS "登录次数"        
FROM ODS_SAP_ZBIT_LOG_BM  
WHERE LEFT(SAL_DATE,6) = :dynamic_param -- '202510' 
      AND KOSTL IS NOT NULL AND KOSTL <> '' AND KOSTL <> '信息中心' 
GROUP BY  KOSTL , LEFT(SAL_DATE,6) 
    ) AS A  
  LEFT JOIN ( 
